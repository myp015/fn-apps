...
Commands:
  ...
  -s, --start [-s|--single-port <port>] [-m|--mirror-port <port>]
      start the chroot environment.
      -s, --single-port <port>
          port number for single desktop noVNC connection, default 6080.
      -m, --mirror-port <port>
          port number for mirror desktop noVNC connection, default 6081.
...
unmount_overlayfs() {
  local abs_overlayfs="${1}"
  mountpoint -q "${abs_overlayfs:-}/merged" && umount "${abs_overlayfs:-}/merged" || true
}
...
x11vnc -auth guess -display :0 -rfbauth /root/.vnc/passwd -rfbport 5901 -forever -shared >/var/log/x11vnc.log 2>&1 &
...
do_stop() {
  ...
  for p in /proc/*/root; do readlink -f "$p" 2>/dev/null | grep -q "^$(realpath "${abs_overlayfs}")" && kill -9 "${p//[^0-9]/}" 2>/dev/null; done
...
do_status() {
  ...
  # Check if the chroot environment is mounted
  if [ -d "${abs_overlayfs:-}/merged" ]; then
    # Check if the desktop environment is running
    if [ -n "$(pgrep -f lightdm)" ]; then
      return 0
    else
      return 3
    fi
...
do_install() {
  ...
  chroot "${abs_overlayfs:-}/merged" env TERM=xterm-256color DEBIAN_FRONTEND=noninteractive bash -c "apt-get update && apt-get install -y dbus-x11 task-xfce-desktop xvfb" || die "Installing XFCE failed: ${abs_overlayfs:-}/merged"
  # Install libpam-mkhomedir and configure PAM to automatically create home directories for new users.
  chroot "${abs_overlayfs:-}/merged" env TERM=xterm-256color DEBIAN_FRONTEND=noninteractive bash -c "apt-get update && apt-get install -y libpam-mkhomedir" || die "Installing libpam-mkhomedir failed: ${abs_overlayfs:-}/merged"
  ...
  chroot "${abs_overlayfs:-}/merged" env TERM=xterm-256color DEBIAN_FRONTEND=noninteractive bash -c "apt-get update && apt-get install -y tigervnc-standalone-server tigervnc-tools x11vnc novnc websockify" || die "Installing TigerVNC server and noVNC failed: ${abs_overlayfs:-}/merged"
  ...
  chroot "${abs_overlayfs:-}/merged" env TERM=xterm-256color DEBIAN_FRONTEND=noninteractive bash <<EOF
apt-get update && apt-get install -y crudini
...
EOF
  ...
  printf '[Seat:*]\nsession-setup-script=/etc/lightdm/usermod-aG.sh\n' >"${abs_overlayfs:-}/merged/etc/lightdm/lightdm.conf.d/50-usermod-aG.conf"
  printf '#!/bin/env bash\n[ -n "$USER" ] && usermod -aG sudo,audio,video,input "$USER"\nexit 0\n' >"${abs_overlayfs:-}/merged/etc/lightdm/usermod-aG.sh"
  chmod a+x "${abs_overlayfs:-}/merged/etc/lightdm/usermod-aG.sh"
